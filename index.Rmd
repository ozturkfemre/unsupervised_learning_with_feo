...
In the ever-evolving landscape of artificial intelligence and machine learning, unsupervised learning stands as one of the most compelling and versatile subjects. It is the cornerstone of algorithms and techniques that enable machines to uncover hidden patterns, structures and knowledge from raw data without the explicit guidance of labeled examples. In this episode, we take a journey to understand the essence of unsupervised learning.

## 1.1. In Search of Hidden Insights

Imagine navigating through a sea of endless streams of data - where texts, images, numbers and more come together in a frenzy. This rich resource of the digital age often carries real value hidden deep within. Traditional machine learning approaches rely on labeled data for training, and it's like navigating with a map. You know your destination and your route is clearly marked. But imagine a treasure map that is handed to you without any explanation, signs or clues. This is where unsupervised learning comes into the picture.

Unsupervised learning is the art of discovering the secrets hidden in the world of data. It involves a set of techniques that allow machines to dive deep into data without knowing what to look for. Instead of being guided by labeled examples, unsupervised learning algorithms dive deep into the ocean of data to uncover underlying structures, relationships and insights. It's like giving a machine a magnifying glass and letting it discover its own patterns and connections.

## 1.2. The Building Blocks of Unsupervised Learning

At the heart of unsupervised learning are two fundamental tasks: clustering and dimensionality reduction. \

### 1.2.1. Clustering 

Clustering is the process of grouping similar observations together while keeping dissimilar ones separate. Think of it as organizing a library without labels; books with similar content are grouped together to form clusters. This technique is extremely useful in areas ranging from customer segmentation in marketing to identifying disease subtypes in healthcare.

### 1.2.2. Dimensionality Reduction

Dimensionality reduction, on the other hand, is about simplifying complex data by reducing the number of features or dimensions. Think of a detailed picture; dimensionality reduction helps create a simpler, abstract representation that preserves the essence of the original. This not only saves computational resources, but also helps visualize high-dimensional data and identify critical features.

## 1.3. Applications of Unsupervised Learning

The applications of unsupervised learning are wide-ranging and continually expanding. From recommendation systems that suggest your next favorite movie to anomaly detection in cybersecurity, unsupervised learning plays a pivotal role in modern technology. It is the foundation of natural language processing, computer vision, and countless other fields.

# 2. Choosing optimal number of clusters

When you come across such a title, you may wonder why it is important to determine number of clusters. first reason is that number of clusters must be predetermined for clustering algorithms to work. For example, many algorithms such as k-means, k-medoids, hierarchical clustering need to know number of clusters in order to work. Depending on data set and work done with that data set, we may know number of clusters in advance. For example, in figure 2.1, it is quite possible to determine number of clusters with a scatter plot.
...
However, determining number of clusters in commonly encountered data sets is too complex to be achieved simply by observing overall structure of data set with a scatter plot. For example, when scatter plot below is analyzed, it is not possible to tell how many clusters data set is divided into. However, since situations like one in above plot are frequently encountered, some methods have been proposed to determine number of clusters. I will now explain what some of these methods are, how they are calculated step by step and their implementation in R.
...
However, since situations like one in above plot are frequently encountered, some methods have been proposed to determine number of clusters. we will cover what some of these methods are, how they are calculated step by step and their implementation in R. In all of examples given, same data set ([Breast Cancer Wisconsin](https://archive.ics.uci.edu/ml/datasets/breast+cancer+wisconsin+%28diagnostic%29)) is used.

## 2.1. Elbow Method

Elbow method, also known as *total within sum of squares*, is a technique used to determine optimal number of clusters for a k-means clustering analysis. idea behind elbow method is to run k-means clustering on data set for a range of values of k (number of clusters), and for each value of k calculate sum of squared distances of each point from its closest centroid (SSE). elbow point is point on plot of SSE against number of clusters (k) where change in SSE begins to level off, indicating that adding more clusters doesn't improve model much. [1], [2]

steps to perform elbow method are:

1.  Select a range of k values, usually from 1 to 10 or square root of number of observations in data set.

2.  Run k-means clustering for each k value and calculate SSE (sum of squared distances of each point from its closest centroid).

3.  Plot SSE for each k value.

4.  point on plot where SSE starts to decrease at a slower rate is elbow point, and corresponding number of clusters is optimal value for k.

Undoubtedly, Elbow method is one of most widely used methods. It can be said to be a method that follows same logic as k-means. However, it would not be correct to say that it gives good results under all circumstances. Sometimes it can even be said to be misleading. For this reason, although I use elbow method in every cluster analysis, I do not rely on it alone.
...
Average silhouette method measures how well-defined a particular cluster is, and how well-separated it is from other clusters. At this point, it is necessary to state that Silhouette value is calculated for each observation in data set. Average of silhouette value of all observations gives us average silhouette value, which is silhouette value of clustering analysis [3] , [4].

steps to calculate silhouette value for a observation are:

1.  calculate cluster tightness: average distance purple observation to all blue observations which all are in same cluster, which is called \[a_{i}\]
...
2.  calculate cluster separation: observation's minimum distance to all observations in a different cluster(yellow cluster), which is called \[b_{i}\]
...
3.  calculate silhouette coefficient: calculate its silhouette value \[s_{i}\] as difference between \[b_{i}\] and \[a_{i}\], divided by maximum of these two distances:

\[s_i=\frac{(b_i-a_i)}{max(b_i,a_i)}\]

After calculating silhouette coefficient of each observation, we can finally, calculate average silhouette value for all observations: mean(\[s_{i}\]). To determine number of clusters, we usually cluster each number of clusters for a range of 2 to 10 clusters and obtain average silhouette value for each number of clusters. silhouette value ranges from -1 to 1, where a value of 1 indicates a strong similarity to other observations in its own cluster, and a value of -1 indicates a strong similarity to observations in another cluster. In other words, number of clusters with highest silhouette value is number of clusters we will determine.

Again `fviz_nbclust` function is a way to see average silhoutte plot to decide optimal number of clusters:
...
Gap Statistic Method compares observed within-cluster variation for different values of k with variation expected under a null reference distribution of data. [5]

steps to perform gap statistic method are:

1.  Select a range of k values, usually from 1 to 10 or square root of number of observations in data set.

2.  Run clustering algorithm (such as k-means or hierarchical clustering) for each k value and calculate within-cluster variation Wk.

3.  Generate B reference data sets by randomly sampling original data and calculate within-cluster variation W\*k for each data set.

4.  Calculate gap statistic

5.  Plot gap statistic for each k value.

6.  k value that corresponds to maximum gap statistic is optimal number of clusters.
...
Just like Average Silhouette Method, Gap Statistic Method is also offers 2 as optimal number of clusters.

## 2.4. Calinski-Harabasz Method

Calinski-Harabasz index (also known as Variance Ratio Criterion) is a commonly used evaluation metric for comparing different clustering solutions in unsupervised learning. It is a ratio of between-cluster variance and within-cluster variance, and it is used to determine number of clusters that should be used in a clustering solution[6].

steps to perform calinski-harabasz method are:

1.  calculate within cluster sum of squares (WCSS): sum of squared distances between each observation and its corresponding cluster center(barycenter).

    ![](https://cdn-images-1.medium.com/max/800/1*4WNGuodDuuGJE_rCGrQwLw.png)

2.  calculate between cluster sum of squares (BCSS): calculate sum of squared distances between each cluster center and overall mean of all observation.
...
Calinski-Harabasz index ranges from 0 to infinity, with a higher value indicating a better clustering. For Calinski - Harabasz method, there is no visualization function in R as in methods I mentioned before. For this reason, lets write a function that calculates and visualizes Calinski - Harabasz values for clusters 2 to 10 using `calinhara` function in `fpc` package.
...
Davies-Bouldin index (DBI) is a measure of similarity between clusters in a clustering solution. DBI is calculated as average similarity between each cluster and its most similar cluster, where similarity between two clusters is defined as maximum distance between any observation in one cluster and its closest observation in other cluster. [7]

steps to perform DBI are:

1.  calculate intra-cluster dispersion: average distance of all data points in each cluster to cluster center.

    ![](https://cdn-images-1.medium.com/max/800/1*aeLLLdSZe-juNaQIm2irbQ.png)

2.  calculate separation criteria: Euclidean distance between cluster centers for each pair of clusters.

    ![](https://cdn-images-1.medium.com/max/800/1*HtFg6se9Yzdt8jE5zRnLmw.png)

3.  find most similar cluster: for each pair of clusters, calculate similarity d(i, j) between two clusters, as defined in previous answer. Sum up maximum similarity between each cluster and its most similar cluster. Divide sum by number of clusters to obtain Davies-Bouldin index.

Davies-Bouldin index ranges from 0 to infinity, with a lower value indicating a better clustering solution. A DBI of 0 indicates that there is no similarity between any two clusters, while a high DBI value indicates that there is a high level of similarity between some of clusters.

Just like Calinski-Harabasz method, there is no visualization function in R as for Davies-Bouldin method. For this reason, I will write a function that calculates and visualizes Davies Bouldin value for clusters 2 to 10 using `NbClust` function in `NbClust` package.
...
Unlike other methods, we see that Davies-Bouldin's suggestion for number of clusters is 7. Although it gives different results in this data set, it can give more reliable results in other data sets. For this reason, it would be useful to include Davies-Bouldin method in every clustering analysis.

## 2.6. Dunn Index

Dunn index is a measure of compactness and separability of clusters in a clustering solution. It is calculated as ratio of minimum separation to maximum diameter. [8]

steps to perform Dunn Index are:

1.  calculate minimum separation: smallest distance between observations from two different clusters.

    ![](https://cdn-images-1.medium.com/max/800/1*EHsIVbk6_82UuuxrKn602Q.png)

2.  calculate maximum diameter: maximum distance between observations in same cluster.
...
Just like Calinski-Harabasz and Davies- Bouldin methods, there is no visualization function in R as for Dunn Index. For this reason, I will write a function that calculates and visualizes Dunn Index values for clusters 2 to 10 using `dunn` function in `clValid` package.
...
3.  Compute Meila's VI as: 
    \[VI=H^1+H^2-2*I\]
     where \[H^1\], \[H^2\] are the entropies of the two clustering solutions, and I is the mutual information between the two clustering solutions.
